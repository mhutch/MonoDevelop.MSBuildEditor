...
namespace MonoDevelop.MSBuild.Editor.VisualStudio.FindReferences
{
	internal partial class StreamingFindUsagesPresenter
	{
		...
		private class WithReferencesFindUsagesContext : AbstractTableDataSourceFindUsagesContext
		{
			...
			protected override async Task OnDefinitionFoundWorkerAsync (FoundReference definition)
			{
			...
			private async Task AddDeclarationEntriesAsync (FoundReference definition)
			{
				...
				lock (Gate) {
					...
					if (!HasDeclarationEntries (definition)) {
						...
						EntriesWhenNotGroupingByDefinition = EntriesWhenNotGroupingByDefinition.Add (definitionEntry);
			...
			private bool HasDeclarationEntries (FoundReference definition)
			{
				lock (Gate) {
			...
			{
				var columnOptions = ImmutableDictionary<string, ImmutableArray<string>>.Empty
					.Add (nameof (ReferenceUsage), ImmutableArray<string>.Empty.Add (nameof (ReferenceUsage)));
				...
				bool whenGroupingByDefinition)
			{
				lock (Gate) {
					...
					return ImmutableArray<FoundReference>.Empty;
				}
			}
			...
			private async Task CreateNoResultsFoundEntryIfNecessaryAsync ()
			{
				...
				if (noDefinitions) {
					...
					await OnEntryFoundAsync (FoundReference.NoResults,
						bucket => SimpleMessageEntry.CreateAsync (
							bucket, "Search found no results"),
						addToEntriesWhenGroupingByDefinition: true,
